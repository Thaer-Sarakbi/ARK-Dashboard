...
export function buildPrompt(reports: AdminReport[]): string {
  const lines = reports
...
export async function fetchAdminReports(dateKey: string): Promise<AdminReport[]> {
...
export async function runAnalysisAndPersist(
  dateKey: string,
  reports: AdminReport[]
): Promise<{ hotelsAnalyzed: number; totalComplaints: number }> {
